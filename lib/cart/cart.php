...
class LibCart
{
    ...
    static function save($name, $email, $idTransporteur, $idAdresse, $quantity, $total) {
        $query = "INSERT INTO panier (name,email, idTransporteur, idAdresse, quantity, total) VALUES (:name, :email, :idTransporteur, :idAdresse, :quantity, :total)";
        self::log()->info(__FUNCTION__, ['query' => $query]);
        $stmt = LibDb::getPDO()->prepare($query);
        $stmt->bindValue(':name', $name);
        $stmt->bindValue(':email', $email);
        $stmt->bindValue(':idTransporteur', $idTransporteur);
